...
MYSQL_USER=$(echo `aws ssm get-parameter --name /uc3/dmp/hub/${1}/DbUsername | jq .Parameter.Value | sed -e "s/\"//g"`)
MYSQL_PASSWORD=$(echo `aws ssm get-parameter --name /uc3/dmp/hub/${1}/DbPassword | jq .Parameter.Value | sed -e "s/\"//g"`)
