...
describe('authCheck', () => {
  ...
  it('it returns null if there is no User for the specified email', async () => {
    ...
    const originalFunc = UserEmail.findByEmail;
    ...
    UserEmail.findByEmail = originalFunc;
  });
  ...
  it('it returns the user\'s id if the password matched', async () => {
    ...
    const originalFunc = UserEmail.findByEmail;
    jest.spyOn(UserEmail, 'findByEmail').mockResolvedValue([
      new UserEmail({ userId: mockUser.id, isPrimary: true, isConfirmed: true, email: email })
    ]);
...
describe('getEmail', () => {
  ...
  it('should return the primary email if it exists', async () => {
    ...
    const originalFunc = UserEmail.findPrimaryByUserId;
    jest.spyOn(UserEmail, 'findPrimaryByUserId').mockResolvedValue(
      new UserEmail({ email: mockEmail })
    );
    ...
    UserEmail.findPrimaryByUserId = originalFunc;
  });

  it('should return null if no primary email exists', async () => {
    const originalFunc = UserEmail.findPrimaryByUserId;
    jest.spyOn(UserEmail, 'findPrimaryByUserId').mockResolvedValue(null);
    const result = await user.getEmail(context);
