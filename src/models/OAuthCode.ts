...
export class OAuthCode implements AuthorizationCode {
  ...
  static async findOne(authorizationCode: string): Promise<OAuthCode | null> {
    try {
      ...
      const user = await User.findById('OAuthClient.findOne', buildContext(logger), row.userId);
      if (!client || !user) {
        return null;
      }
      return new OAuthCode({
        ...
        user: await User.findById('OAuthClient.findOne', buildContext(logger), row.userId),
      });
