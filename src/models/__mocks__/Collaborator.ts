...
// Clean up all mock/test TemplateCollaborator
export const cleanUpAddedTemplateCollaborator = async (
  context: MyContext,
  id?: number,
) : Promise<void> => {
  const reference = 'cleanUpAddedTemplateCollaborator';
  try {
    // Do a direct delete on the MySQL model because the tests might be mocking the
    // TemplateCollaborator functions
    await TemplateCollaborator.delete(context, TemplateCollaborator.tableName, id, reference);
  } catch (e) {
    console.error(`Error cleaning up template collaborator id ${id}: ${e.message}`);
    if (e.originalError) console.log(e.originalError);
  }
}
...
export const cleanUpAddedProjectCollaborator = async (
  ...
  const reference = 'cleanUpAddedProjectCollaborator';
  try {
    // Do a direct delete on the MySQL model because the tests might be mocking the
    // ProjectCollaborator functions
