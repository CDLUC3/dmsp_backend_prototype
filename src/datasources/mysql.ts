...
export class MySQLConnection implements DatabaseConnection {
    ...
    constructor() {
        ...
        try {
            ...
            throw new DatabaseError('Failed to create connection pool', err instanceof Error ? err : undefined);
        }
    }
    ...
    public async getConnection(): Promise<mysql2.PoolConnection> {
        try {
            ...
            throw new DatabaseError('Failed to get connection from pool', err instanceof Error ? err : undefined);
        }
    }
    ...
    public async query<T>(context: MyContext, sql: string, values: string[] = []): Promise<T> {
        ...
        try {
            ...
            throw new DatabaseError('Database query failed', err instanceof Error ? err : undefined);
        } finally {
            if (connection) {
                connection.release();
            }
    ...
    public async close(): Promise<void> {
        if (this.pool) {
            try {
                ...
                throw new DatabaseError('Failed to close connection pool', err instanceof Error ? err : undefined);
            }
        }
    }
}
